...
8. [mandb](#mandb)
9. [type](#type)
10. [Переклад довідки (на прикладі української мови)](md#%D0%BF%D0%B5%D1%80%D0%B5%D0%BA%D0%BB%D0%B0%D0%B4-%D0%B4%D0%BE%D0%B2%D1%96%D0%B4%D0%BA%D0%B8-%D0%BD%D0%B0-%D0%BF%D1%80%D0%B8%D0%BA%D0%BB%D0%B0%D0%B4%D1%96-%D1%83%D0%BA%D1%80%D0%B0%D1%97%D0%BD%D1%81%D1%8C%D0%BA%D0%BE%D1%97-%D0%BC%D0%BE%D0%B2%D0%B8)
11. [Створення власної довідки](#%D1%81%D1%82%D0%B2%D0%BE%D1%80%D0%B5%D0%BD%D0%BD%D1%8F-%D0%B2%D0%BB%D0%B0%D1%81%D0%BD%D0%BE%D1%97-%D0%B4%D0%BE%D0%B2%D1%96%D0%B4%D0%BA%D0%B8-man)
...
## Створення власної довідки man

інструкція на прикладі власної команди `command`

1. Підготовка тексту (формат roff)
   
  Man-сторінки пишуться спеціальною мовою розмітки `groff` (або `nroff`). Створіть файл `command.1` (номер 1 означає першу секцію — команди користувача):
    ```bash
    nano command.1
    ```
  
  Вставте туди базовий шаблон:
    ```roff
    .TH GCOM 1 "16 Лютого 2024" "1.0" "Мій власний мануал"
    .SH NAME
    command \- крута власна команда для автоматизації
    .SH SYNOPSIS
    .B gcom
    [\fIопція\fR] [\fIфайл\fR]
    .SH DESCRIPTION
    Команда \fBgcom\fR робить чудові речі, які я сам запрограмував.
    .SH OPTIONS
    .TP
    .BR \-v, \-\-verbose
    Виводити детальну інформацію під час роботи.
    .SH EXAMPLES
    .B command -v
    .SH AUTHOR
    Ваше Ім'я
...
Система man віддає перевагу стиснутим файлам для економії місця (хоча може прочитати і звичайний .1).
```bash
gzip -k command.1
```

У вас з'явиться файл `command.1.gz`.

3. Розміщення у системній папці
    
Для копіювання в `/usr/share/man/man1/` потрібні права суперкористувача:
```bash
sudo cp command.1.gz /usr/share/man/man1/
```

<details>
  <summary>
    Докладніше про вибір папки 
  </summary>
  
  Вибір папки залежить від того, що саме ви документуєте. Система man використовує ці цифри (секції), щоб розрізняти команди, системні функції та формати файлів.
    
  Ось детальний гайд, як обрати правильну папку:
   
  1. Короткий довідник по секціях
   
      | Папка	| Для чого призначена	| Приклади |
      | ------ | ------------------ | -------- |
      | man1	| Команди користувача	| Ваші скрипти, програми, ls, command |
      | man2	| Системні виклики	| Функції ядра (наприклад, open, fork, kill) |
      | man3	| Бібліотечні функції	| Функції мови C (наприклад, printf, strcpy) |
      | man4	| Спеціальні файли	| Пристрої в /dev (наприклад, null, random) |
      | man5	| Формати файлів	| Конфіги (наприклад, /etc/passwd, hosts, nanorc) |
      | man6	| Ігри	| Текстові ігри та заставки |
      | man7 |	Різне	| Макроси, протоколи (наприклад, ip, utf8, regex) |
      | man8	| Адміністрування	| Команди для root (наприклад, fdisk, reboot, iptables) |
  
  2. Як обрати для вашої команди gcom?
      
      - Якщо це звичайна програма або скрипт, які запускає будь-який користувач — кладіть у man1. Це стандартне місце для 99% власних розробок.
      - Якщо це команда виключно для root (наприклад, для керування сервером чи мережею) — кладіть у man8.
      - Якщо command — це конфігураційний файл (наприклад, ~/.command.conf), то опис його структури має лежати в man5.
  
  3. Чому це важливо?
     
      Інколи назви збігаються. Наприклад:
      - `man 1 printf` — покаже документацію до команди в терміналі.
      - `man 3 printf` — покаже документацію до функції в мові C.
      
      Якщо ви покладете свій мануал у man1, користувач зможе відкрити його просто як `man command`. Якщо ж ви вирішите створити ще й опис конфігураційного файлу для цієї програми, ви покладете його в man5, і доступ до нього буде через man 5 command.
  
  4. Де саме шукати ці папки?
     
      Система шукає мануали в кількох місцях, які визначені в файлі `/etc/manpath.config` (можна переглянути командою manpath). Найпопулярніші:
      - `/usr/share/man/` — для програм, встановлених менеджером пакетів.
      - `/usr/local/share/man/` — рекомендоване місце для ваших власних ручних мануалів (щоб вони не перемішалися з системними).
      
      Порада: Перед тим як копіювати файл, переконайтеся, що назва файлу закінчується на номер секції. Наприклад: command.1.
  
      Офіційні рекомендації щодо ієрархії файлової системи можна знайти в The Filesystem Hierarchy Standard (FHS).
    
      Ось фінальний чек-лист, щоб ваш мануал став «рідним» для системи:
    
      a) Перевірте назву файлу
      - Назва файлу мусить закінчуватися на номер секції.
      - Правильно: `command.1` або `command.1.gz`.
      - Неправильно: `command.gz` (система не зрозуміє, до якої секції його віднести).
    
      b) Заголовок всередині файлу
      - Перший рядок у файлі (розмітка roff) має збігатися з номером папки:
        ```groff
        .TH COMMAND 1 ... — де 1 це і є секція.
        ```
      c) Куди краще копіювати?
      - Хоча `/usr/share/man/man1/` спрацює, за стандартами Filesystem Hierarchy Standard (FHS) власноруч створене ПЗ краще класти в:
      `/usr/local/share/man/man1/`
      - Це вбереже ваші файли від випадкового видалення під час оновлення операційної системи.
      
      d) Винятки (коли НЕ man1)
      - `man5`: Якщо ви написали круту програму gcom, але хочете окремо описати, як влаштований її файл налаштувань `/etc/command.conf`. Тоді опис параметрів конфігу кладіть у man5.
      - `man8`: Якщо ваша команда настільки потужна, що може зламати систему, і призначена тільки для адміністратора (потребує sudo).
      
      Як швидко перевірити результат?
      
      Не обов'язково щоразу копіювати файл. Ви можете подивитися, як він буде виглядати в man ще на етапі написання:
        ```bash
        man ./command.1
        ```
  
      Маленька хитрість: Якщо ви створили функцію-аліас у `.bashrc` (наприклад, `alias com='command'`), ви можете зробити так, щоб  `man com` теж відкривав ваш мануал. Для цього створіть файл `com.1` з одним рядком:
      ```
      .so man1/command.1
      ```
      Це працює як "символьне посилання" всередині системи man.
  
</details>

4. Оновлення бази даних
   
Щоб команди `whatis` та `apropos` теж "побачили" ваш новий мануал, оновіть базу:
  ```bash
  sudo mandb
  ```
...
**Альтернативний (простіший) спосіб**

Якщо ви не хочете копіювати файл у системні папки, ви можете просто вказати man шлях до файлу:
  ```bash
  man ./command.1.gz
  ```

Порада: Якщо розмітка `roff` здається занадто складною, існує інструмент pandoc, який дозволяє написати документацію у звичайному Markdown і конвертувати її в формат man:
  ```bash
  pandoc doc.md -s -t man -o command.1
  ```
